...
peso_risco <- function(x){0.5 + 5*x} # como é calculado o peso de risco de um empréstimo. x é PD - faz parte da equação (2)
...
mutuarios <- list(valor_emprestimo = matrix(NA,ncol = num_mutuarios, nrow = t),
                  ...
                  peso_risco = matrix(NA,ncol = num_mutuarios, nrow = t), # faz parte da equação (2)
...
familias$qual_banco[1,] <- sample(1:num_bancos, 
                                  size = num_familias, 
                                  replace = TRUE)
  
familias$prob_mudar_banco[1,] <- runif(ncol(familias$prob_mudar_banco), 
                                       prob_min_mudar_banco, 
                                       prob_max_mudar_banco)

for (o in 2:t) {
  familias$prob_mudar_banco[o,] <- familias$prob_mudar_banco[1,]
}

mutuarios$prob_default[1:t,] <- runif(ncol(mutuarios$prob_default), 
                                      pd_min, pd_max)

mutuarios$tx_recuperacao[1,] <- tx_recuperacao
mutuarios$peso_risco[1,] <- peso_risco(mutuarios$prob_default[1,]) # faz parte da equação (2)

mutuarios$qual_banco[1,] <- sample(1:num_bancos, 
                                   size = num_mutuarios, replace = TRUE)

mutuarios$fim_emprestimo[1,] <- sample(1:periodos_emprestimos, 
                                       size = num_mutuarios, replace = TRUE)

# calculando o custo inicial de capital ------- (1) o juros aqui é a tx de retorno livre de risco

for (q in 1:num_mutuarios) {
  
  mutuarios$custo_capital[1,q] <- (1 + ((margem_emprestimo[1]+1) *juros[1]) - 
                                     mutuarios$prob_default[1,q] * 
                                     mutuarios$tx_recuperacao[1,q])/(1-mutuarios$prob_default[1,q])
...
for (w in 1:num_bancos) {
  ...
  # soma de depósitos - Fazendo apenas para linha 1 e para todas as colunas (w)
          # O processo se repetirar logo mais para p-1 linhas e para todas as colunas (e)
        
  bancos$soma_depositos[1,w] <- 
    sum(familias$valor_depositos[1,which(familias$qual_banco[1,] == w)]) 
  ...
  risco_potencial_mutuario_cart_emprest <- 
    mutuarios$peso_risco[1,which(mutuarios$qual_banco[1,] == w)]
  
  # número potencial de empréstimos que o banco possui depósitos para emprestar com reserva ------- 
      # fundos máximos a emprestar (dadas as reservas mínimas e os requisistos de adequação de capital)
  reserva_potencial_Market_Share <- 
    bancos$soma_depositos[1,w] * (1-ind_reserv_min[1])
  
  # número potencial de empréstimos que estarão em conformidade com os regulamentos 
                # obs: (10.1) no bancos$patrimonio[1,w]+2) < bancos$patrimonio[1,w])[2]
  participacao_potencial_mercado <- 
    table(c(cumsum(
      risco_potencial_mutuario_cart_emprest * ind_adeq_capital[1]),
      bancos$patrimonio[1,w]+2) < bancos$patrimonio[1,w])[2] # o valor patrimonial adicionado em c() é porque sempre precisa haver um exemplo falso
  
  # número de empréstimos que atendem a dois requisitos (reservas e requisitos de capital) 
                #------- (10) - completo "potencial de emprestimos"
  potencial_emprestimos <- 
    floor(min(reserva_potencial_Market_Share, 
              participacao_potencial_mercado))
  
  # conceder empréstimos a esses mutuários sem alguma margem de segurança
  mutuarios$valor_emprestimo[1,which(mutuarios$qual_banco[1,] == w)] <- 
    c(rep(1,(potencial_emprestimos-5)), 
      rep(0,(length(risco_potencial_mutuario_cart_emprest)+5-potencial_emprestimos)))
 
  
   # soma dos empréstimos 
          # obs: dos mutuarios "mutuarios$valor_emprestimo" para os bancos "bancos$carteira_emprestimo[1,w]"
            
  bancos$carteira_emprestimo[1,w] <-
    sum(mutuarios$valor_emprestimo[1,which(mutuarios$qual_banco[1,] == w)])
  
  # calcular ativos ponderados pelo risco parte da equação (5) obs: entre parenteses da equação em questão
  bancos$ativo_ponderados_risco[1,w] <- 
    sum(mutuarios$peso_risco[1,which(mutuarios$qual_banco[1,] == w)]*
          mutuarios$valor_emprestimo[1,which(mutuarios$qual_banco[1,] == w)])
  
  # provisões para perdas esperadas (EL)--------  - obs: somente para a 1a linha
      # EL = PD * LGD * EAD
      # LGD = 1 - tx de recuperação
  bancos$prov_perdas_esperadas[1,w] <- 
    sum(mutuarios$valor_emprestimo[1,which(mutuarios$qual_banco[1,] == w)] *
          mutuarios$prob_default[1,which(mutuarios$qual_banco[1,] == w)] *
          (1-mutuarios$tx_recuperacao[1,which(mutuarios$qual_banco[1,] == w)]))
}
...
for (p in 2:t) {
  ...
  for (u in 1:num_familias) {
    ...
    nova_prob_mudar_banco <- familias$prob_mudar_banco[1,u] + 
      qual_seguro[familias$qual_banco[p,u]] # adiciona efeito de segurança 
    
    mudanca_banco[u] <- sample(c(1,0),size = 1,replace = TRUE,
                               prob = c(nova_prob_mudar_banco, 1-nova_prob_mudar_banco))
  }
  
  # gerando aleatoriamente um novo banco para cada depósito mudar 
          # (a família pode mudar de banco para o mesmo banco)
  familias$qual_banco[p,which(mudanca_banco == 1)] <- sample(which(bancos$falencia[p-1,] == 0), 
                                                             replace = TRUE, 
                                                             size = length(which(mudanca_banco == 1))) 
  ...
  for (e in which(bancos$falencia[p-1,] == 0)) {
    bancos$soma_depositos[p,e] <- 
      sum(familias$valor_depositos[p,which(familias$qual_banco[p,] == e)])
  }
  ...
  for (i in which(mutuarios$valor_emprestimo[p,] == 1)) {
    qual_inadimplencia[i] <- sample(c(1,0), size = 1,
                                    replace = TRUE, prob = c(mutuarios$prob_default[p,i], 
                                                             1-mutuarios$prob_default[p,i]))
  }
  # empréstimos inadimplentes não valem mais nada
          # (exceto a recuperação inicial, calculada posteriormente)
  ...
  ## ABBA é CLoss eq-6
  perda_emprestimo_t <- bancos$carteira_emprestimo[p,] - 
    bancos$carteira_emprestimo[p-1,]
  
  # atualização das provisões para perdas esperadas --------  - obs: Para o restante
  # EL = PD * LGD * EAD
  # LGD = 1 - tx de recuperação
  mutuarios$tx_recuperacao[p,] <- mutuarios$tx_recuperacao[p-1,]
  for (a in which(bancos$falencia[p-1,] == 0)) {
    
    bancos$prov_perdas_esperadas[p,a] <- 
      sum(mutuarios$prob_default[p,which(mutuarios$qual_banco[p,] == a)] * 
  ...
  # obs: Reservas = ( Somatorio dos depositos * tx de juros dos depositos ) - 
  # Variação da prov. das perdas esperadas - (Somatório conj. emprestimos* tx de juros paga pelo empréstimo)
  bancos$reservas[p,] <- bancos$soma_depositos[p,] - 
    bancos$carteira_emprestimo[p,] + bancos$patrimonio[p-1,] 
  
  
  
  # índice mininmo de reserva ------- 
            # qual banco deve vender empréstimos por causa das reservas mínimas
            # (1 se cumprirem os requisitos, 0 caso contrário)
  satisfazer_reserva <- ifelse(bancos$reservas[p,] < (bancos$soma_depositos[p,] * 
                                                        ind_reserv_min[p]),0,1) # ------- 
  ...
  for (d in which(satisfazer_reserva == 0)) {
    ...
    num_min_emprestimos_venda <- 
      ceiling((bancos$soma_depositos[p,d]*ind_reserv_min[p] - 
                 bancos$reservas[p,d])/venda_reserva_emergencia) # número mínimo de empréstimos para vender
    
    perda_venda_forcada[d] <- 
      ceiling(bancos$soma_depositos[p,d]*ind_reserv_min[p] - 
                bancos$reservas[p,d]) * (1-venda_reserva_emergencia) # para cálculo adicional de PnL
    
    emprestimos_bancarios <- 
      mutuarios$valor_emprestimo[p,which(mutuarios$qual_banco[p,] == d & 
                                           mutuarios$valor_emprestimo[p,] == 1)]
    
    mutuarios$valor_emprestimo[p,which(mutuarios$qual_banco[p,] == d & 
                                         mutuarios$valor_emprestimo[p,] == 1)] <- 
      c(rep(1,length(emprestimos_bancarios) -
              num_min_emprestimos_venda), 
  ...
  margem_deposito[p] <- margem_deposito[p] - 
    ind_concentracao((num_bancos-sum(bancos$falencia[p]))/num_bancos)
  
  margem_emprestimo[p] <- margem_emprestimo[p] + 
    ind_concentracao((num_bancos-sum(bancos$falencia[p]))/num_bancos)
  ...
  for (l in 1:num_mutuarios) {
    ...
    mutuarios$custo_capital[p,l] <- 
      ((1 + ((margem_emprestimo[p]+1)*juros[p]) - 
          mutuarios$prob_default[2,l] * 
          mutuarios$tx_recuperacao[p,l])/(1-mutuarios$prob_default[2,l]))
  ...
  for (y in which(bancos$falencia[p-1,] == 0)) {
    ...
    receita_loop <- sum(mutuarios$custo_capital[p-1, 
                                                mutuarios$qual_banco[p,] == y] * 
                          mutuarios$valor_emprestimo[p, 
                                                     mutuarios$qual_banco[p,] == y]) - 
      sum(mutuarios$valor_emprestimo[p,
                                     mutuarios$qual_banco[p,] == y])  # menos comprimento porque usamos uma fórmula 1+juros
    
    # somando os juros das reservas investidas durante o período anterior e menos a venda imediata 
    receita_loop <- receita_loop + 
      (bancos$reservas[p-1,y] * juros[p-1])
  ...
  for (f in which(bancos$falencia[p-1,] == 0)) {
    ...
    bancos$custo[p,f] <- 
      sum(familias$valor_depositos[p-1, 
                                   which(familias$qual_banco[p,] == f)] * 
            juros[p] * (1+margem_deposito[p])) + 
      perda_venda_forcada[f]
  }
  
  # custo de assistência à liquidez
  bancos$custo[p,] <- bancos$custo[p,] + 
    (bancos$assist_liquidez[p-1,] * juros[p])
  
  # lucro líquido dos bancos
  bancos$renda[p,] <- (bancos$receita[p,] - bancos$custo[p,]) + 
    perda_emprestimo_t - (bancos$prov_perdas_esperadas[p-1,] - 
                            bancos$prov_perdas_esperadas[p,]) - perda_venda_forcada
  
  # Colocar o "-perda_emprestimo_t" e "-reservas"
  bancos$patrimonio[p,] <- bancos$patrimonio[p-1,] + 
    bancos$renda[p,]
  ...
  for (g in which(bancos$falencia[p-1,] == 0)) {
    ...
    bancos$ativo_ponderados_risco[p,g] <- sum(mutuarios$valor_emprestimo[p,
                                                                         which(mutuarios$qual_banco[p,] == g)] * 
                                                mutuarios$peso_risco[p,
                                                                     which(mutuarios$qual_banco[p,] == g)])
    
    maximo_dividendo <- bancos$patrimonio[p,g] - 
      bancos$ativo_ponderados_risco[p,g]*
      (ind_adeq_capital[p]*
         lim_super_ind_adeq_capital)
    
    # segundo requisito é que o banco possa dar dividendos se a reserva mínima permitir
    maximo_dividendo_depositos <- bancos$reservas[p,g] - 
      (bancos$soma_depositos[p,g] * 
         (ind_reserv_min[p] * 
            reservas_metas))
    
    # se um deles não permitir dar dividendos (ou seja, 0), então dê zero
    bancos$dividendos[p,g] <- min(pmax(c(maximo_dividendo, 
                                         maximo_dividendo_depositos),0))
    
    bancos$patrimonio[p,g] <- bancos$patrimonio[p,g] - 
      min(pmax(c(maximo_dividendo, 
                 maximo_dividendo_depositos),0))
  }
  
  # atualizar reservas após diminuir o patrimônio líquido em um dividendo
  bancos$reservas[p,] <- bancos$patrimonio[p,] + 
    bancos$soma_depositos[p,] - 
    bancos$carteira_emprestimo[p,]
  ...
  for (k in which(bancos$falencia[p,] == 1)) {
    ...
    familias$valor_depositos[p, 
                             which(familias$qual_banco[p,] == k)] <- 0
    
    # empréstimos para venda imediata
    bancos$carteira_emprestimo[p:t,k] <- 0
    
    mutuarios$valor_emprestimo[p, 
                               which(mutuarios$qual_banco[p,] == k)] <- 0
    
    # permitir que outros bancos concedam empréstimos aos seus clientes
    mutuarios$qual_banco[p, 
                         which(mutuarios$qual_banco[p,] == k)] <- 
      sample(which(bancos$falencia[p,] == 0), 
             size = length(mutuarios$qual_banco[p, 
                                                which(mutuarios$qual_banco[p,] == k)]),
                                                                            replace = TRUE)
    
    #permitir que outros bancos recebam depósitos de seus clientes
    familias$qual_banco[p,
                        which(familias$qual_banco[p,] == k)] <- 
      sample(which(bancos$falencia[p,] == 0), 
             size = length(familias$qual_banco[p,
                                               which(familias$qual_banco[p,] == k)]),
                                                                         replace = TRUE)
  }
  
  # verificando a subcapitalização
  bancos$subcapitalizado[p,] <-  
    ifelse(bancos$patrimonio[p,] < (bancos$ativo_ponderados_risco[p,] * 
                                      ind_adeq_capital[p]), 1,0)
  
  for (j in which(bancos$subcapitalizado[p,] == 1)) {
    
    if(bancos$falencia[p,j] == 1){next()}
    
    necessid_liquidez <- (bancos$ativo_ponderados_risco[p,j]*
                            ind_adeq_capital[p]) - 
      bancos$patrimonio[p, j]
  ...
  for (h in which(bancos$falencia[p,] == 0)) {
    ...
    excesso_reservas <- bancos$reservas[p,h] - 
      (bancos$soma_depositos[p,h] * 
         ind_reserv_min[p])
    
    excesso_capital <- (bancos$patrimonio[p,h] - 
                          (bancos$ativo_ponderados_risco[p,h] * 
                             ind_adeq_capital[p]))/mean(mutuarios$peso_risco[p,])
    
    if(any(c(excesso_capital, excesso_reservas) < 0)){next()}
    
    potencial_emprestimos <- min(pmax(c(excesso_reservas, 
                                        excesso_capital), 0))
    
    # se o mercado potencial for menor que os empréstimos potenciais, omitir
    if(length(mutuarios$valor_emprestimo[p,mutuarios$valor_emprestimo[p,] == 0 & 
                                         mutuarios$qual_banco[p,] == h]) < floor(potencial_emprestimos)){
      potencial_emprestimos <- length(mutuarios$valor_emprestimo[p,mutuarios$valor_emprestimo[p,] == 0 & 
                                                                   mutuarios$qual_banco[p,] == h])-2
    }
    # substitua os empréstimos do banco que são 0 por 1s dados aleatoriamente.
    mutuarios$valor_emprestimo[p,mutuarios$valor_emprestimo[p,] == 0 & 
                                 mutuarios$qual_banco[p,] == h] <- 
      sample(c(rep(1, floor(potencial_emprestimos)), 
               rep(0,length(mutuarios$valor_emprestimo[p,mutuarios$valor_emprestimo[p,] == 0 
                                                       & mutuarios$qual_banco[p,] == h]) - 
                     floor(potencial_emprestimos))),
             replace = FALSE)
    
    # alterar carteira de empréstimos
    bancos$carteira_emprestimo[p,h] <- 
      sum(mutuarios$valor_emprestimo[p, which(mutuarios$qual_banco[p,] == h)]) 
  ...
  for (s in 1:num_bancos) {
    ...
    bancos$ativo_total[p, ] <- bancos$patrimonio[p-1,] + 
      bancos$soma_depositos [p-1,] +
      bancos$carteira_emprestimo [p-1,] + 
      bancos$reservas [p-1,]
  ...
  for (z in 1:num_bancos) {
    ...
    bancos$market_share[p, ] <- 
      bancos$carteira_emprestimo[p-1,] / sum(bancos$carteira_emprestimo[p,])
